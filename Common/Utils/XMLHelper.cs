...
namespace Common.Utils
{
    public static class XMLHelper
    {
        ...
        /// <param name="xpath">要匹配的XPath表达式(例如:"//节点名//子节点名")</param>
        /// <returns>返回XmlNode</returns>
        public static string GetXmlNodeValueByXpath(string xmlFileName, string xpath, string name, string initialValue)
        {
            ...
            try
            {
                ...
                {
                    XmlElement subElement = xmlDoc.CreateElement(name);
                    subElement.InnerXml = initialValue;
                    xmlNode.AppendChild(subElement);
                }
                var node = xmlNode.SelectSingleNode(name).FirstChild;
            ...
            {
                // CreateXmlDocument(xmlFileName, xpath, "1.0", "utf-8", "no");
        ...
        /// <param name="innerText">节点文本值</param>
        /// <returns>成功返回true,失败返回false</returns>
        public static bool CreateOrUpdateXmlNodeByXPath(string xmlFileName, string xpath, string innerText)
        {
            bool isSuccess = false;
            bool isExistsNode = false;//标识节点是否存在
            XmlDocument xmlDoc = new XmlDocument();
            try
            {
                ...
                if (xmlNode != null)
                {
                    ...
                    //没有节点信息
                  //  xmlNode.
                  //XmlAttribute xa =  xmlDoc.CreateAttribute(xpath);
                  //  xa.Value = innerText;
                }
                xmlDoc.Save(xmlFileName); //保存到XML文档
                isSuccess = true;
